...
<nav class="navbar">
  ...
  <div class="user-menu">
    ...
    if(!isset($_SESSION['ad']) && !isset($_SESSION['parol'])){
        echo'
        <div style="font-family: Tahoma, sans-serif;" class="navbar-account"> <a target="_blank" href="hesab.php">GİRİŞ <i class="lni lni-user"></i> </a></div>
        <div class="navbar-account"> <a target="_blank" href="qeydiyyat.php">QEYDİYYAT <i class="lni lni-user"></i> </a></div> ';   
    }
    else{
        ...
        <div style="font-family: Tahoma, sans-serif; font-weight:bold; " class="navbar-account">'.$info['ad'].'<i class="lni lni-user"></i></div>';
    }
    ?>        
    <!-- end navbar-account --> 
  </div>
  ...
  <div class="container">
    <h6 style="font-family: Tahoma, sans-serif;">Film axtarin</h6>
     <form method="post" action="axtar.php">
      ...
      <a class="btn btn-danger btn-lg" href="film.php">Geri</a>
    </form>
    <!--
    <ul>
...
<main>
  <section class="content-section">
    ...
    <div class="row">
        ...
        <!-- end col-12 -->
        <?php
        $sec = mysqli_query($con,"SELECT *,movies.id, video_cdn.id as movie_id FROM movies,video_cdn where movies.original_title=video_cdn.orig_title and 
            (title like '%".$_POST['axtar']."%' or  overview like '%".$_POST['axtar']."%') GROUP BY movies.original_title
            order by release_date desc limit 1,24");
  ...
  <!-- end content-section -->
    ...
    <div class="row">
        <div class="col-12">
          <div class="section-title">
            <h2 style="font-family: Tahoma, sans-serif;">İmdbsi ən yüksək olan filmlər</h2>
          </div>
          <!-- end section-title --> 
        </div>
        <!-- end col-12 -->
       <?php
        $sec = mysqli_query($con,"SELECT *,movies.id, video_cdn.id as movie_id FROM movies,video_cdn 
            where original_title not in('Fetish','Positive','Yes','Toast','Kiss Kiss Bang Bang','Songbird','A Brush with Love','Passing') 
            and movies.original_title=video_cdn.orig_title and (title like '%".$_POST['axtar']."%' or  overview like '%".$_POST['axtar']."%')
            GROUP BY movies.original_title order by vote_average desc limit 1,24");
        while($info = mysqli_fetch_array($sec))
        {
        ?>
        <div class="col-xl-2 col-lg-3 col-md-4 col-sm-6 col-6">
