...
<!-- SOCIAL MEDIA META -->
<meta property="og:description" content="digiflex | Ontline Movie Streaming">
...
<section class="search-box">
  <div class="container">
    ...
    <form method="post" action="axtar.php">
      ...
      <a class="btn btn-danger btn-lg" href="index.php">Geri</a>
    </form>
    <!--
    <ul>
      <li><a href="#">Pulp Function</a></li>
  ...
  <section class="content-section" data-background="#111111">
    <div class="container">
      <div class="row">
        ...
        <!-- end col-12 -->
        <?php
        //$sec = mysqli_query($con,"SELECT * FROM movies order by release_date desc limit 1,24");
        ...
        if(isset($_GET['id'])){
            ...
            $sec = mysqli_query($con,"SELECT *,movies.id, video_cdn.id as movie_id FROM movies,video_cdn where movies.original_title=video_cdn.orig_title
            GROUP BY movies.original_title order by release_date desc limit 97,192");
            while($info = mysqli_fetch_array($sec)){ 
        ?>        
           ...
           <a href="film-single.php?id=<?=$info['movie_id']?>"><figure class="video-image"> <img src="<?='https://www.themoviedb.org/t/p/w220_and_h330_face'.$info['poster_path']?>" alt="Image"> 
            ...
            <div class="video-content"> <small class="range"></small>
              <ul class="tags">
                <li>
                <?php
                $jsec = mysqli_query($con,"SELECT * FROM janr WHERE gid='".$info['genre']."'");
                $jinfo = mysqli_fetch_array($jsec);
                echo'<div class="tags">'.$jinfo['janr'].'</div>';
                ?>
                </li>
              </ul>
        ...
        <!-- end col-2 -->
        <div class="col-12 text-center"> <a style="font-family: Tahoma, sans-serif;" href="index.php?id" class="custom-button">Daha çox</a> </div>
        <!-- end col-12 --> 
      </div>
      <!-- end row --> 
    </div>
